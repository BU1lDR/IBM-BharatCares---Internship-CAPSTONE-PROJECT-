...
# One build timestamp, used both for the date printed in the document and for the
# dcterms:created/modified properties, so those two can never disagree the way
# they used to. SOURCE_DATE_EPOCH is honoured because this script is the one
# artefact here whose output is otherwise not a pure function of facts.json:
# export it and the .docx becomes reproducible.
_EPOCH = os.environ.get("SOURCE_DATE_EPOCH")
BUILD_DT = (datetime.fromtimestamp(int(_EPOCH), timezone.utc) if _EPOCH
...
REPO_URL = "https://github.com/BU1lDR/retail-customer-segmentation"
...
# The four lines above were the only properties ever set, which left the rest of
# them as whatever python-docx's bundled default.docx happens to carry. That is
# not nothing: the file claimed dcterms:created and dcterms:modified of
# 2013-12-23T23:15:00Z, an empty lastModifiedBy, and a dc:description reading
# "generated by python-docx". Right-click > Properties on the report showed a
# document apparently written in December 2013, eleven years before the dataset
# it analyses was downloaded and twelve before the internship. Nobody reads those
# fields often, but a marker whose whole purpose is provenance should not be
# saying something false, and the body of this document already prints the real
# build date two pages in — so the properties disagreed with the page.
doc.core_properties.created = BUILD_DT
doc.core_properties.modified = BUILD_DT
doc.core_properties.last_modified_by = "Aryan Verma"
doc.core_properties.comments = (
    "Generated by tools/build_report.py. Every number is read from "
    "outputs/facts.json and every chart from outputs/figures/, both written by a "
    f"full run of {NOTEBOOK}."
)
doc.core_properties.category = "Capstone project report"
...
BUILD_DATE = BUILD_DT.strftime("%d %B %Y")
...
# write
# --------------------------------------------------------------------------- #
def rewrite_app_properties(path):
    """Replace the docProps/app.xml python-docx inherits from its own template.

    core_properties above covers docProps/core.xml, but nothing in the python-docx
    API reaches app.xml, so it shipped through untouched from the bundled
    default.docx: `Application` read "Microsoft Macintosh Word", `AppVersion`
    14.0000, `Template` Normal.dotm, and `Pages`/`Words`/`Characters` were 1/0/0.
    Every one of those is false. This document was not written in Word, on a Mac
    or otherwise, and a report about data hygiene is a poor place to leave a field
    that says otherwise.

    The counts are dropped rather than corrected. Word recomputes them when it
    opens the file, and this script cannot know a page count without laying the
    document out — an omitted optional element is honest in a way that a 0 is not.
    """
    ...
    app_xml = (
        ...
        f"<Application>python-docx {DOCX_VERSION}</Application>"
    ...
    # Entries are copied with their original ZipInfo, so the archive's internal
    # timestamps and compression are preserved and only the one part changes.
...
doc.save(OUT_PATH)
rewrite_app_properties(OUT_PATH)
